...
def apply_qpe(circuit, control_register, target_register, public_key_x, public_key_y, p):
    n_count = len(control_register)
    circuit.x(target_register[0])
    modular_exponentiation_operator(circuit, control_register, target_register, public_key_x, public_key_y, n_count, p)
...
def apply_qft(circuit, num_qubits):
    if num_qubits > circuit.num_qubits:
        raise ValueError("num_qubits must be less than or equal to the number of qubits in the circuit.")
...
def create_qldpc_code(num_qubits):
    # Create a quantum circuit for the QLDPC code
    qldpc_circuit = QuantumCircuit(num_qubits)

    for i in range(num_qubits):
        if i % 2 == 0:  # Example condition for parity
            qldpc_circuit.h(i)  # Apply Hadamard to even-indexed qubits

    return qldpc_circuit

def create_shyps_code(num_qubits):

    shyps_circuit = QuantumCircuit(num_qubits)

    for i in range(num_qubits):
        if i % 2 == 0:  # Example condition for encoding
            shyps_circuit.h(i)  # Apply Hadamard to even-indexed qubits

    return shyps_circuit

def quantum_key_recovery(num_qubits, public_key_hex, p):
    ...
    cr = ClassicalRegister(num_qubits + 4, 'c')
    circuit = QuantumCircuit(qr, aux, ancilla, cr)
    
    # Create QLDPC code
    qldpc_circuit = create_qldpc_code(num_qubits)
    circuit.append(qldpc_circuit, qr)  # Append QLDPC encoding to the circuit

    create_superposition(circuit, qr)
    circuit.barrier()
    apply_qpe(circuit, qr, aux, public_key_x, public_key_y, p)
    circuit.barrier()
...
def run_quantum_search(public_key_hex, num_qubits, num_shots, max_attempts, p):
    logger.info(f"Using backend: {backend.name}")
    results = []

    for attempt in range(1, max_attempts + 1):
        try:
            qc = quantum_key_recovery(num_qubits, public_key_hex, p)
    ...
    if period:
        ...
        if verify_private_key(period, public_key_hex, G[0], G[1], p):
...
def quantum_circuit(num_qubits, Gx, Gy, Px, p):
    ...
    for i in range(num_qubits):
        k = 2 ** i
        Qx, Qy = scalar_multiplication(k, Gx, Gy, p)
    ...
    # Apply QPE (which includes controlled unitaries and inverse QFT)
    apply_qpe(qc, q_x, qc_aux, Gx, Gy, p) 
...
def main():
    ...
    num_qubits = int(np.ceil(np.log2(max_range))) #17 #18 #19 #20 #25 #35 #int(np.ceil(np.log2(max_range))) + 1
    ...
    quantum_registers = int(np.ceil(np.log2(max_range))) + 1 #17 #18 #19 #20 #25 #35 #int(np.ceil(np.log2(max_range))) + 1
    num_shots = 8192
    max_attempts = 1
    ...
    qc = quantum_key_recovery(num_qubits, public_key_x_hex, p)
    ...
    if period:
        padded_key = hex(period)[2:].zfill(64)
        if verify_private_key(period, public_key_x_hex, G[0], G[1], p):
    ...
    private_key = run_quantum_search(public_key_x_hex, num_qubits, num_shots, max_attempts, p)
    if private_key:
